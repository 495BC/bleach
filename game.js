...
const Game = {
  ...
  init() {
    ...
    window.addEventListener('keydown', e => {
      ...
      if (!this.els.arrowPanel.classList.contains('hidden')) {
        const key = e.key.startsWith('Arrow') ? e.key : 'Arrow' + e.key;
  ...
  newGame() {
    ...
    if (faction === 'Soul Reaper') {
      const tpl = ZanpakutoDB[name] || ZanpakutoDB.Ichigo;
      zp = new Zanpakuto({
        name: tpl.name,
        shikaiSkills: tpl.shikai.map(s => new Skill(s)),
        bankaiSkills: tpl.bankai.map(s => new Skill(s))
      });
    }
  ...
  initSkills() {
    const p = this.player;
    p.skills = p.faction === 'Soul Reaper'
      ? [ new Skill({ name:'Zanjutsu Slash', cost:0, power:1, desc:'Basic blade', unlockLv:1 }) ]
      : [ new Skill({ name:'Hollow Claw',    cost:0, power:1, desc:'Ferocious claw', unlockLv:1 }) ];
  },
  ...
  startArrancarMini() {
    ...
    this.arrowSeq = Array.from({ length: 4 }, () =>
      ['ArrowUp','ArrowDown','ArrowLeft','ArrowRight'][rand(4)]
    );
    this.arrowInput = [];
    this.els.arrowsDiv.innerText = this.arrowSeq.map(a => `[${a.replace('Arrow','')}]`).join(' ');
