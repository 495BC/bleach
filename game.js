...
class Character {
  constructor({ name, hp, reiatsu, atk, def, spd, zanpakuto = null, faction }) {
    ...
    this.maskOn     = false;
    // start with basic blade skill
    this.skills     = [ new Skill({
      name: 'Zanjutsu Slash',
      cost: 0,
      power: 1.0,
      desc: 'Basic blade strike'
    })];
    // inventory skeleton
    this.inventory  = [];
  }
  ...
  learnShikai() {
    if (this.zanpakuto && !this.hasShikai) {
  ...
  learnBankai() {
    if (this.zanpakuto && !this.hasBankai) {
...
const Game = {
  ...
  log(msg) {
    // msg may contain trusted HTML (we control it), so use innerHTML
  ...
  playerTurn(idx) {
    ...
    let dmg = this.calcDamage(this.player, this.enemy, sk.power);
  ...
  enemyTurn() {
    ...
    if (this.enemy.reiatsu < sk.cost) {
      ...
      let dmg = this.calcDamage(this.enemy, this.player, sk.power);
  ...
  levelUp() {
    ...
    if (p.lv === 15) { this.log('Hollow Mask attainable!'); }
  ...
  save() {
    ...
    localStorage.setItem('BleachRPGSave', data);
    alert('Game saved.');
  },
  ...
  load() {
    ...
    try {
      ...
      return alert('Save file corrupted.');
    }
    // rehydrate
    ...
    // skills array → Skill instances
    this.player.skills    = (raw.skills || []).map(s => new Skill(s));
    // inventory stub
    this.player.inventory = raw.inventory || [];
    // refresh UI
    this.updatePlayerPanel();
    this.populateSkills();
    alert('Game loaded.');
